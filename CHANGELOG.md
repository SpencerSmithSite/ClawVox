...
### Added
- **D-02** Installer creation: `component-pkg` and `pkg` Makefile targets; `scripts/distribution.xml` (productbuild distribution XML, requires macOS 13+, installs to `/Applications`); `scripts/pkg-scripts/preinstall` (quits app before install); usage: `make pkg [INSTALLER_CERT="Developer ID Installer: ..."] [VERSION=1.0]` produces `build/ClawVox-<version>.pkg` via `pkgbuild` (component package) → `productbuild` (flat distributable installer)
